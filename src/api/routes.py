...
# Phase 3 Advanced Inbox Optimization Endpoints

@app.post("/campaigns/{campaign_id}/inbox-placement-test")
async def run_inbox_placement_test(campaign_id: int, db=Depends(get_db_session)):
    """Run comprehensive inbox placement test with seed accounts"""
    try:
        from ..core.inbox_placement_tester import InboxPlacementTester
        ...
        results = await tester.pre_send_test(campaign_id, test_content)

        return {
            "campaign_id": campaign_id,
            "placement_test_results": results,
            "recommendation": "Proceed with send" if results.get('safe_to_send') else "Optimize before sending",
            "next_steps": [
                "Review recommendations" if not results.get('safe_to_send') else "Send to engagement boosters first",
                "Monitor real-time metrics during send",
                "Check ISP feedback loops for complaints"
            ]
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Inbox placement test failed: {str(e)}")

@app.post("/campaigns/{campaign_id}/spam-test")
async def run_comprehensive_spam_test(campaign_id: int, db=Depends(get_db_session)):
    """Run comprehensive spam testing with multiple services"""
    try:
        from ..integrations.spam_testing_services import SpamTestingServices
        ...
        results = await spam_tester.run_comprehensive_test(campaign_data)

        return {
            "campaign_id": campaign_id,
            "spam_test_results": results,
            "service_status": spam_tester.get_service_status(),
            "overall_recommendation": "Safe to send" if results.get('safe_to_send') else "Optimize content before sending"
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Spam testing failed: {str(e)}")

@app.get("/authentication/verify")
async def verify_authentication_setup():
    """Verify all authentication methods (SPF, DKIM, DMARC, BIMI, etc.)"""
    try:
        from ..core.advanced_authentication import AdvancedAuthentication

        auth_checker = AdvancedAuthentication("cjsinsurancesolutions.com")
        verification_results = await auth_checker.verify_all_authentication()

        return {
            "authentication_verification": verification_results,
            "setup_recommendations": verification_results.get('recommendations', []),
            "security_level": verification_results.get('security_level', 'Unknown'),
            "next_steps": [
                "Fix any failed authentication methods",
                "Consider implementing optional security features",
                "Monitor authentication results in ISP feedback"
            ]
        }
...
@app.post("/authentication/setup-bimi")
async def setup_bimi_configuration():
    """Get BIMI setup configuration and requirements"""
    try:
        from ..core.advanced_authentication import AdvancedAuthentication

        auth_setup = AdvancedAuthentication("cjsinsurancesolutions.com")
        bimi_config = auth_setup.setup_bimi_record()

        return {
            "bimi_configuration": bimi_config,
            "implementation_priority": "High - Improves brand recognition in Gmail",
            "estimated_cost": "$1,299-$1,499 per year for VMC certificate",
            "roi_benefits": [
                "Increased open rates (5-10% improvement)",
                "Enhanced brand trust",
                "Protection against spoofing",
                "Premium inbox appearance"
            ]
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"BIMI setup failed: {str(e)}")

@app.post("/authentication/setup-mta-sts")
async def setup_mta_sts_configuration():
    """Get MTA-STS setup configuration"""
    try:
        from ..core.advanced_authentication import AdvancedAuthentication

        auth_setup = AdvancedAuthentication("cjsinsurancesolutions.com")
        mta_sts_config = auth_setup.setup_mta_sts()

        return {
            "mta_sts_configuration": mta_sts_config,
            "implementation_priority": "Medium - Enhanced security",
            "security_benefits": [
                "Forces TLS encryption",
                "Prevents downgrade attacks",
                "Improves enterprise delivery",
                "Demonstrates security commitment"
            ]
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"MTA-STS setup failed: {str(e)}")

@app.get("/feedback-loops/status")
async def get_feedback_loop_status(db=Depends(get_db_session)):
    """Get ISP feedback loop setup status"""
    try:
        from ..core.feedback_loop_manager import FeedbackLoopManager

        fbl_manager = FeedbackLoopManager(db)
        status = fbl_manager.get_setup_status()

        return {
            "feedback_loop_status": status,
            "setup_priority": "Critical - Required for reputation monitoring",
            "benefits": [
        ...
        raise HTTPException(status_code=500, detail=f"Feedback loop status check failed: {str(e)}")

@app.post("/feedback-loops/register")
async def register_feedback_loops(db=Depends(get_db_session)):
    """Register with all ISP feedback loops"""
    try:
        from ..core.feedback_loop_manager import FeedbackLoopManager

        fbl_manager = FeedbackLoopManager(db)
        registration_results = await fbl_manager.register_all_feedback_loops()
        ...
        raise HTTPException(status_code=500, detail=f"Feedback loop registration failed: {str(e)}")

@app.get("/feedback-loops/metrics")
async def get_isp_metrics(db=Depends(get_db_session)):
    """Get current metrics from all ISP feedback loops"""
    try:
        from ..core.feedback_loop_manager import FeedbackLoopManager

        fbl_manager = FeedbackLoopManager(db)
        metrics = await fbl_manager.get_isp_metrics()

        return {
            "isp_metrics": metrics,
            "data_freshness": "Real-time where available",
            "monitoring_status": "Active" if metrics.get('isps') else "Setup Required"
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"ISP metrics retrieval failed: {str(e)}")

@app.post("/feedback-loops/webhook/{isp}")
async def process_feedback_loop_webhook(isp: str, webhook_data: dict, db=Depends(get_db_session)):
    """Process incoming feedback loop webhook data"""
    try:
        from ..core.feedback_loop_manager import FeedbackLoopManager

        fbl_manager = FeedbackLoopManager(db)
        await fbl_manager.process_feedback_loop_data(isp, webhook_data)

        return {
            "status": "processed",
            "isp": isp,
            "processed_at": datetime.utcnow().isoformat(),
            "data_points": len(webhook_data) if isinstance(webhook_data, dict) else 1
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Webhook processing failed: {str(e)}")

@app.get("/inbox-optimization/dashboard")
async def get_inbox_optimization_dashboard(db=Depends(get_db_session)):
    """Get comprehensive inbox optimization dashboard"""
    try:
        from ..core.inbox_placement_tester import InboxPlacementTester
        from ..core.advanced_authentication import AdvancedAuthentication
        from ..core.feedback_loop_manager import FeedbackLoopManager
        ...
        placement_tester = InboxPlacementTester(db)
        placement_history = placement_tester.get_historical_performance(30)

        # Get authentication status
        auth_checker = AdvancedAuthentication("cjsinsurancesolutions.com")
        auth_status = await auth_checker.verify_all_authentication()

        # Get feedback loop status
        fbl_manager = FeedbackLoopManager(db)
        fbl_status = fbl_manager.get_setup_status()

        # Calculate overall optimization score
        placement_score = placement_history.get('average_inbox_rate', 0) if isinstance(placement_history, dict) else 85
        auth_score = auth_status.get('overall_score', 0)
        fbl_score = fbl_status.get('completion_percentage', 0)

        overall_score = (placement_score + auth_score + fbl_score) / 3

        return {
            "optimization_dashboard": {
                "overall_score": round(overall_score, 1),
                "placement_performance": placement_history,
                "authentication_status": auth_status,
                "feedback_loop_status": fbl_status,
            ...
            "priority_actions": [
                action for action in [
                    "Fix authentication issues" if auth_score < 80 else None,
                    "Improve content quality" if placement_score < 80 else None,
                    "Complete feedback loop setup" if fbl_score < 80 else None
